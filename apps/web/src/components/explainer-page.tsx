...
import { getExplainer } from "@/lib/api";
...
export async function ExplainerPage({
  countrySlug,
  explainerSlug,
}: ExplainerPageProps) {
  const explainer = await getExplainer(countrySlug, explainerSlug);

  if (!explainer) {
    notFound();
  }

  const isFlagExplainer = explainer.slug === "old-nigerian-flag";
  ...
  return (
    <PageShell>
      ...
      <Container className="py-12 lg:py-16">
        <div className="mb-8">
          <Link
            ...
            href={`/countries/${explainer.country.slug}`}
          >
            Back to {explainer.country.name}
          </Link>
        </div>
        <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <p className="mt-5 text-base font-semibold text-[#5d665f]">
              Updated {formatDate(explainer.publishedAt)} •{" "}
              {explainer.sources.length || "Visible"} sources •{" "}
              {explainer.readTime}
            </p>
          </div>
          <Card className="bg-[#fbfcf8]">
            <p className="text-sm font-bold uppercase tracking-wide text-[#00703c]">
              {isFlagExplainer ? "Image comparison" : "Country context"}
            </p>
            {isFlagExplainer ? (
              <div className="mt-6 flex flex-wrap gap-6">
                <div>
                  <ColonialNigeriaFlag className="h-28 w-48" />
                  <p className="mt-3 text-sm font-semibold text-[#5d665f]">
                    Before independence
                  </p>
                </div>
                <div>
              ...
              <div className="mt-6">
                ...
                <p className="mt-4 text-lg font-semibold">
                  {explainer.country.officialName ?? explainer.country.name}
                </p>
                <p className="mt-2 leading-7 text-[#3f4842]">
                  {explainer.topic?.description ??
                    "A short explainer connected to national identity, history, and everyday curiosity."}
                </p>
              </div>
            )}
          </Card>
      ...
      <Container className="grid gap-10 py-14 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <Eyebrow>{isFlagExplainer ? "Timeline" : "Deeper context"}</Eyebrow>
          <h2 className="mt-3 text-4xl font-bold tracking-tight">
            {isFlagExplainer
              ? "The flag story in three steps"
              : "The simple version, with context"}
          </h2>
        </div>
        {isFlagExplainer ? (
          ...
          <div className="grid gap-5">
            {explainer.body.split("\n").map((paragraph) => (
              <p key={paragraph} className="text-lg leading-8 text-[#3f4842]">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          <div className="grid gap-3">
            {explainer.sources.length ? (
              explainer.sources.map((source) => (
                <a
                  key={source.url}
                  className="rounded border border-[#3a463f] px-4 py-3 font-semibold text-[#dce5dc] underline underline-offset-4"
                  href={source.url}
                >
                  {source.title}
              ...
              <div className="rounded border border-[#3a463f] px-4 py-3 font-semibold text-[#dce5dc]">
                Sources will be attached before public launch.
              </div>
            )}
            <p className="mt-3 text-sm leading-6 text-[#b9c3ba]">
              LoreSabi stores source links and image attribution with each
              explainer, so AI-assisted drafts can still be reviewed like a
              proper knowledge product.
            </p>
          </div>
        </Container>
      </section>
      ...
      <Container className="py-12">
        <PrimaryLink href={`/countries/${explainer.country.slug}`}>
          Explore more {explainer.country.name} explainers
        </PrimaryLink>
      </Container>
    </PageShell>
  );
